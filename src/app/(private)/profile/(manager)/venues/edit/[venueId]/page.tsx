...
import { updateManagerVenue } from '@/api/venues/updateManagerVenue';
...
export default function EditVenuePage() {
  ...
  const onSubmit = async (data: CreateVenueData) => {
    ...
    try {
      const updatedVenue = await updateManagerVenue(venue.id, data);
      if (updatedVenue) {
        setIsSaved(true);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setVenues(
        managerUser ?? '',
        (venues ?? []).filter((item) => item.id !== venue.id),
      );
      router.push('/profile/venues');
    } catch (error) {
      setErrorMessage(
